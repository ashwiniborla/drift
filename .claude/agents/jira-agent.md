...
```
OPERATION: SETUP_EPIC | CREATE_STORY | CREATE_SUBTASK | CLOSE_STORY | ADD_COMMENT | ATTACH_DOC | SCOPE_CHANGE_NOTIFY | SETUP_AUTH
...
Creates a Jira Sub-task under a parent story. Used by integration-tests agent (one sub-task per test case) and by planner Phase 2C-3 (one sub-task per `_till_done.json` subtask).

**Required fields:** `PARENT_KEY`, `TITLE`, `DESCRIPTION`, `FEATURE_TAG`

```bash
RESULT=$(bash scripts/agent/jira.sh create-subtask \
  "$PARENT_KEY" "$TITLE" "$DESCRIPTION" 2>&1)

if echo "$RESULT" | grep -q "^SUBTASK_CREATED:"; then
  SUBTASK_KEY=$(echo "$RESULT" | sed -nE 's/^SUBTASK_CREATED:([^:]+):.*/\1/p')
  SUBTASK_URL=$(echo "$RESULT" | sed -nE 's/^SUBTASK_CREATED:[^:]+:(.*)$/\1/p')
  echo "JIRA_SUBTASK_CREATED:${SUBTASK_KEY}:${SUBTASK_URL}"
else
  echo "JIRA_ERROR:SUBTASK_CREATE_FAILED"
  echo "$RESULT" >&2
  exit 1
fi
```

The caller is responsible for persisting the returned key (e.g., to `_test_jira_map.json` for integration tests, or to `_till_done.json` for planner subtasks). This operation does NOT write to harness-state.md.
...
| `JIRA_SUBTASK_CREATED:<key>:<url>` | Sub-task created | Persist key to caller's map (e.g. `_test_jira_map.json`), proceed |
