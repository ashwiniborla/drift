...
Most repos carry a `sonar.properties` file (or `sonar-project.properties`) at the repo root or in a recognised sub-path. **Always attempt to read it before relying on any caller-supplied INPUT or `harness-state.md` values.**
...
if [ -n "$SONAR_PROPS" ]; then
  ...
  PROPS_PROJECT_NAME=$(grep -oP '^\s*sonar\.projectName\s*=\s*\K.*' "$SONAR_PROPS" 2>/dev/null | xargs || echo "")
  ...
  [ -z "$PROPS_HOST_URL" ] && [ -n "$_raw_url" ] && \
    echo "WARNING: sonar.host.url '$_raw_url' in $SONAR_PROPS is not a known Flipkart Sonar instance — ignoring." >&2
fi
```

Use values extracted from `sonar.properties` as follows:

| Property | Extracted value | Applied to |
|---|---|---|
| `sonar.projectKey` | `PROPS_PROJECT_KEY` | Used as `INPUT` for RESOLVE_PROJECT; used in place of `SONAR_PROJECT` in RUN_GATE skip check and trigger steps |
| `sonar.host.url` | `PROPS_HOST_URL` | Used as `sonar-base-url` override (preferred over default) |
| `sonar.projectName` | `PROPS_PROJECT_NAME` | Informational — log only, not persisted |

**Priority order for project key resolution:**
1. `sonar.properties` → `sonar.projectKey` (highest — source of truth in the repo)
2. `harness-state.md` → `sonar-project-key` (already resolved in a prior session)
3. Caller-supplied `INPUT` (URL or project key from user/dispatcher)

If `PROPS_PROJECT_KEY` is non-empty and `sonar-project-key` in `harness-state.md` is absent or differs, **auto-update `harness-state.md`** without asking the user:
...
Log: `sonar.properties found at <path> — project key: <PROPS_PROJECT_KEY>, host: <PROPS_HOST_URL>`
...
Credentials are saved globally to ~/.harness/secrets/sonar.credentials and
...
If `PROPS_PROJECT_KEY` was extracted in Step 0 (above), skip the user-supplied `INPUT` entirely and use `PROPS_PROJECT_KEY` directly as the resolved key. Jump to Step 3 (Persist). No API call needed — the repo already declares its own project key.

**Step 1 — Auth check** (see above). Runs whether key came from `sonar.properties` or `INPUT`.

**Step 2 — Resolve** (only when `PROPS_PROJECT_KEY` is empty)

Use `INPUT` from the caller (user-supplied URL or project key):
...
```bash
# Prefer sonar.properties key (already set in harness-state.md by Entry Protocol Step 0 above),
# then fall back to any pre-existing harness-state.md value.
SONAR_PROJECT=$(grep -oP 'sonar-project-key:\s*\K\S+' harness-state.md 2>/dev/null || echo "")
# Final fallback: PROPS_PROJECT_KEY from sonar.properties (if Entry Protocol ran)
